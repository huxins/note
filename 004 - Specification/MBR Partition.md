...
[MBR](https://en.wikipedia.org/wiki/Master_boot_record)，又叫做主引导扇区，是计算机开机后访问硬盘时所必须要读取的首个扇区，它在硬盘上的三维地址为（柱面，磁头，扇区）＝（0，0，1）。
...
- **磁盘簇**
  ...
  在 Windows 下如 NTFS 等文件系统中叫做簇；在 Linux 下如 Ext4 等文件系统中叫做块。每个簇或块可以包括 2、4、8、16、32、64 等 2 的 n 次方个扇区。
